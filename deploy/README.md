# Deploy `wa.risetcenter.com` (VPS apps)
...
Cloudflare (Proxied, SSL Flexible) ──HTTP:80──► edge nginx (survey-populicenter)
        wa.risetcenter.com.conf ──► wa_frontend (Nginx SPA) ──► wa_backend (Fastify) ─┐
                                                                  wa_worker ───────────┤
...
- Cert `origin.pem` (SAN `*.risetcenter.com`) di `/var/www/survey-populicenter/certs`.
...
#     FRONTEND_ORIGIN=https://wa.risetcenter.com
...
cp deploy/wa.risetcenter.com.conf /var/www/survey-populicenter/wa.risetcenter.com.conf
...
curl -s -H "Host: wa.risetcenter.com" http://127.0.0.1/health
```
Lalu buka **https://wa.risetcenter.com** → login `populi` / `populi13!` (ganti setelah masuk).
