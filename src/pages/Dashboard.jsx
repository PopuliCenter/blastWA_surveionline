...
import { PageHeader, StatCard, Card, Badge, Loading, Notice, Empty, useLoader, useIsMobile, theme, fmtDate, Icon } from "../lib/ui";
  ...
  return (
    <div style={{ marginBottom: 13 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", fontSize: 12.5, marginBottom: 5 }}>
        <span style={{ color: theme.textMuted }}>{label}{hint ? <span style={{ fontSize: 11, marginLeft: 5 }}>· {hint}</span> : null}</span>
        <span style={{ color: theme.text, fontWeight: 700 }}>{pct}% <span style={{ color: theme.textMuted, fontWeight: 400, fontSize: 11.5 }}>({(value || 0).toLocaleString("id-ID")})</span></span>
      </div>
...
function Mini({ label, value, tone, icon }) {
  return (
    <div style={{ flex: "1 1 92px", background: theme.surfaceAlt, borderRadius: 10, padding: "11px 12px", textAlign: "center" }}>
      {icon ? <div style={{ color: tone || theme.textMuted, marginBottom: 3 }}><Icon name={icon} size={16} /></div> : null}
  ...
  return (
    <div style={{ marginBottom: 9 }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 8, fontSize: 12, marginBottom: 3 }}>
        <span style={{ color: theme.text, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", maxWidth: "68%" }}>{label}</span>
      ...
      <div style={{ height: 9, background: theme.surfaceAlt, borderRadius: 999, overflow: "hidden" }}>
        <div style={{ height: "100%", width: `${value > 0 ? Math.max(pct, 4) : 0}%`, background: tone, borderRadius: 999 }} />
      </div>
    </div>
  );
}

const QUALITY = { GREEN: ["green", "Tinggi (Hijau)"], YELLOW: ["yellow", "Sedang (Kuning)"], RED: ["red", "Rendah (Merah)"], UNKNOWN: ["default", "Belum dinilai"] };
...
export default function Dashboard() {
  ...
  return (
    <div>
      <PageHeader title="Dashboard" subtitle="Ringkasan operasional WhatsApp & survei Anda." actions={[
        <span key="r" onClick={() => { stats.reload(); surveys.reload(); blasts.reload(); consent.reload(); policy.reload(); convos.reload(); quality.reload(); }} style={{ cursor: "pointer", color: theme.textMuted, display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13 }}><Icon name="refresh" size={16} />Refresh</span>,
      ]} />

      {/* Kartu ringkas */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(210px,1fr))", gap: 16, marginBottom: 16 }}>
        <StatCard label="Pesan Terkirim" value={(s.sent || 0).toLocaleString("id-ID")} note={`${pctOf(s.delivered, s.sent)}% sampai • ${pctOf(s.opened, s.sent)}% dibaca`} tone="green" icon="broadcast" />
        <StatCard label="Total Kontak" value={(s.contacts || 0).toLocaleString("id-ID")} note={`${s.segments} segmen • ${cd.optedOut ?? 0} opt-out`} tone="blue" icon="contacts" />
        <StatCard label="Survei" value={s.surveys} note={`${s.activeSurveys ?? 0} aktif • ${s.responses} respons`} tone="purple" icon="survey" />
        <StatCard label="Gagal Kirim" value={s.failed} note={attempted ? `${pctOf(s.failed, attempted)}% dari percobaan` : "belum ada kirim"} tone={s.failed > 0 ? "yellow" : "default"} icon="autoreply" />
      </div>

      {/* Funnel + Kesehatan */}
      <div style={grid2}>
        <Card title="Funnel Pengiriman">
          {attempted === 0 ? <Empty icon="broadcast" title="Belum ada pengiriman" note="Kirim broadcast dulu untuk melihat rasio." /> : (
            <>
              ...
              <div style={{ fontSize: 11.5, color: theme.textMuted, marginTop: 4, lineHeight: 1.5 }}>Basis: {(s.sent || 0).toLocaleString("id-ID")} terkirim, {attempted.toLocaleString("id-ID")} percobaan. Rasio "dibaca" bergantung setelan read-receipt penerima.</div>
            </>
          )}
        </Card>
        ...
        <Card title="Kesehatan Akun (anti-banned)">
          {/* Consent */}
          <div style={{ fontSize: 12, fontWeight: 700, color: theme.text, marginBottom: 8 }}>Status Langganan Kontak</div>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12.5, marginBottom: 5 }}>
            ...
            <span style={{ color: theme.text, fontWeight: 700 }}>{used} / {limit || "∞"}</span>
          </div>
          <div style={{ height: 8, background: theme.surfaceAlt, borderRadius: 999, overflow: "hidden", marginBottom: 14 }}>
            <div style={{ height: "100%", width: `${Math.min(100, usedPct)}%`, background: usedPct > 90 ? theme.red : usedPct > 70 ? theme.yellow : theme.green, borderRadius: 999 }} />
          </div>
          {/* Kualitas nomor Meta */}
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", background: theme.surfaceAlt, borderRadius: 10, padding: "10px 12px" }}>
            <div>
              <div style={{ fontSize: 12, color: theme.textMuted }}>Kualitas nomor (Meta)</div>
              <div style={{ fontSize: 12.5, color: theme.text, marginTop: 2 }}>{q?.display_phone_number || (quality.loading ? "memuat…" : "belum tersambung")}{q?.messaging_limit_tier ? ` • tier ${String(q.messaging_limit_tier).replace("TIER_", "")}` : ""}</div>
            </div>
      ...
      <div style={grid2}>
        <Card title="Ringkasan Inbox & Survei">
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 14 }}>
            ...
            <Mini label="Belum dibalas" value={inbox.unread} tone={inbox.unread ? theme.red : theme.textMuted} icon="autoreply" />
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
            ...
            <Mini label="Penyelesaian" value={`${pctOf(s.responsesCompleted, s.responses)}%`} tone={theme.blue} icon="report" />
          </div>
        </Card>
        ...
        <Card title="Performa Blast Terakhir">
          {bl.length ? (
            <div>
              {bl.map((b) => (
                <BarRow key={b.id} label={b.surveyTitle && b.surveyTitle !== "-" ? b.surveyTitle : (b.message || b.segmentName || "Blast")} value={b.sent || 0} max={blMax} tone={theme.primary} sub={`${b.sent || 0}✓ · ${b.opened || 0} dibaca`} />
              ))}
            </div>
      ...
      <div style={grid2}>
        <Card title="Respons per Survei">
          {svByResp.length && svByResp.some((v) => v.responses) ? (
            <div>
              {svByResp.map((v) => (
                <BarRow key={v.id} label={v.title} value={v.responses || 0} max={svMax} tone={theme.purple} sub={`${v.responses || 0} respons`} />
              ))}
            </div>
              ...
              {(surveys.data || []).slice(0, 5).map((sv) => (
                <div key={sv.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 12px", background: theme.surfaceAlt, borderRadius: 10 }}>
                  <div>
                    ...
                    <div style={{ color: theme.textMuted, fontSize: 12, marginTop: 2 }}>{sv.questions.length} pertanyaan • {sv.responses} respons{sv.mode === "flow" ? " • Flow" : ""}</div>
                  </div>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            {(blasts.data || []).slice(0, 6).map((b) => (
              <div key={b.id} style={{ padding: "10px 12px", background: theme.surfaceAlt, borderRadius: 10 }}>
                ...
                <div style={{ color: theme.textMuted, fontSize: 12, marginTop: 2 }}>Terkirim {b.sent} • Sampai {b.delivered} • Dibaca {b.opened}{b.failed ? ` • Gagal ${b.failed}` : ""}</div>
              </div>
            ))}
          </div>
        ) : <Empty icon="broadcast" title="Belum ada blast" />}
