...
export default function Admin({ currentUser }) {
  ...
  return (
    <div>
      <PageHeader
        title="Admin"
        subtitle="Kelola user & akses dashboard."
        actions={[
          <Button key="r" variant="ghost" icon="refresh" onClick={reload}>
            Refresh
          </Button>,
          <Button key="n" icon="plus" onClick={() => setModalUser({})}>
            Tambah User
          </Button>,
        ]}
      />
            ...
            {users.map((u, i) => (
              ...
              >
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  <Button variant="secondary" size="sm" icon="edit" onClick={() => setModalUser(u)}>
                    Edit
                  </Button>
                  {u.id !== currentUser.id ? (
                    <>
                      <Button
                        variant="ghost"
                        size="sm"
                        onClick={() => run(() => api.updateUser(u.id, { active: !u.active }))}
                      >
...
function UserModal({ user, onClose, onSave }) {
  ...
  const submit = async () => {
    setSaving(true);
    try {
      const d = { name: f.name, username: f.username, email: f.email, role: f.role, active: f.active };
      if (f.password.trim()) d.password = f.password;
      await onSave(d);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <Modal title={user ? "Edit User" : "Tambah User"} onClose={onClose}>
      ...
      />
      <label
        style={{ display: "flex", alignItems: "center", gap: 9, marginBottom: 16, color: theme.text, fontSize: 13 }}
      >
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 8 }}>
        ...
        <Button
          onClick={submit}
          disabled={!f.name.trim() || !f.username.trim() || (!user && !f.password.trim()) || saving}
        >
