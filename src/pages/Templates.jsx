  ...
  return (
    <div>
      <PageHeader title="Template Pesan" subtitle="Buat & kelola template WhatsApp untuk broadcast. Template wajib disetujui Meta sebelum dipakai." actions={[
        <Button key="r" variant="ghost" icon="refresh" onClick={() => tpls.reload()}>Refresh</Button>,
      ...
      <Card style={{ marginBottom: 16 }} pad={16}>
        <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
          ...
          <div style={{ fontSize: 13, color: theme.textMuted, lineHeight: 1.6 }}>
            <strong style={{ color: theme.text }}>Cara kerja:</strong> susun template di sini → tunggu disetujui Meta → tandai statusnya <strong>Disetujui</strong> → pilih saat membuat Blast.
            Template dipakai untuk <strong>memulai percakapan</strong> (broadcast ke orang yang belum membalas). Untuk balasan dalam 24 jam terakhir, pakai chat biasa tanpa template.
          </div>
        </div>
      </Card>
      ...
      {tpls.loading ? <Loading /> : (tpls.data || []).length ? (
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fill,minmax(320px,1fr))", gap: 14 }}>
          {(tpls.data || []).map((t) => <TemplateCard key={t.id} t={t} onEdit={() => setEditing(t)} onDelete={() => run(() => api.deleteTemplate(t.id))} onDuplicate={() => setEditing({ ...t, id: undefined, name: `${t.name}_copy`, status: "draft" })} />)}
        </div>
...
function TemplateEditor({ initial, onClose, onSave }) {
  ...
  const form = (
    <div>
      ...
      {/* Status */}
      <div style={{ borderTop: `1px solid ${theme.border}`, margin: "6px 0 12px" }} />
      <Select label="Status di Meta" value={f.status} onChange={(e) => set("status", e.target.value)} options={Object.entries(STATUS_LABEL).map(([value, label]) => ({ value, label }))} />
      <div style={{ fontSize: 11.5, color: theme.textMuted, marginTop: -8, marginBottom: 4 }}>Tandai <strong>Disetujui</strong> setelah Meta menyetujui template, agar bisa dipilih di Blast.</div>
    </div>
  );
