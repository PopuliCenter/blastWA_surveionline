...
export function ContactImporter({ onContacts }) {
  ...
  useEffect(() => {
    onContacts(current); /* eslint-disable-next-line */
  }, [mode, raw, contacts]);
    ...
    <div>
      ...
      {mode === "file" ? (
        <>
          ...
          <div style={{ background: theme.surfaceAlt, borderRadius: 10, padding: 12, marginBottom: 12 }}>
            <div style={{ fontSize: 12.5, color: theme.text, fontWeight: 600, marginBottom: 8 }}>
              Unduh template (pilih sesuai kebutuhan):
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
              <button onClick={downloadTemplateBiasa} style={tplCard}>
                ...
                <span style={{ fontSize: 11, color: theme.textMuted, lineHeight: 1.35 }}>
                  Nama + No hp saja. Untuk blast cepat.
                </span>
              </button>
              <button onClick={downloadTemplatePembobot} style={tplCard}>
                ...
                <span style={{ fontSize: 11, color: theme.textMuted, lineHeight: 1.35 }}>
                  + demografi (umur, agama, pekerjaan…). Tergabung ke hasil survei.
                </span>
              </button>
            </div>
          </div>
          ...
          <div
            ...
            style={{
              ...
              textAlign: "center",
              cursor: "pointer",
              background: dragging ? theme.primarySoft : theme.surface,
          ...
          >
            ...
            {fileName ? (
              ...
              <div style={{ color: theme.textMuted, fontSize: 13.5 }}>
                Seret file ke sini atau <span style={{ color: theme.primary, fontWeight: 600 }}>klik untuk pilih</span>
              </div>
            )}
            <div style={{ color: theme.textMuted, fontSize: 12, marginTop: 6 }}>
              Format: .xlsx, .xls, .csv • Wajib: Nama, No hp • Opsional: kolom pembobot apa pun
            </div>
            <input
              ref={fileRef}
              type="file"
                ...
                return (
                  ...
                  >
                    ...
                    {attrKeys.length ? (
                      <span>
                        {" "}
                        • {attrKeys.length} kolom pembobot: {attrKeys.slice(0, 6).join(", ")}
                        {attrKeys.length > 6 ? "…" : ""}
                      </span>
                    ) : (
                      <span> • tanpa pembobot</span>
                    )}
        ...
        <>
          ...
          />
          <div style={{ color: theme.textMuted, fontSize: 12.5, marginBottom: 6 }}>
